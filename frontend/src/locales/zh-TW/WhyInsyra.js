...
filtered.Show()`,
        ...
        cclExample: {
            title: 'CCL 範例：條件與衍生欄位',
            pandas: `# pandas: 使用 lambda 與 assign 新增欄位
df = pd.DataFrame({
    "A": [85,92,78,95,88],
    "B": [20,22,19,23,21]
})
...
df["Grade"] = df["A"].apply(lambda x: "優秀" if x>90 else ("良好" if x>80 else "及格"))
print(df)`,
            insyra: `// Insyra CCL: 使用 AddColUsingCCL 與 CASE 函數
...
dt.AddColUsingCCL("等級", "CASE(A > 90, '優秀', A > 80, '良好', '及格')")
dt.Show()`
        },
