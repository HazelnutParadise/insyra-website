...
filtered.Show()`,
        ...
        cclExample: {
            title: 'CCL Example: Conditional & Derived Columns',
            pandas: `# pandas: use lambda and assign to add columns
df = pd.DataFrame({
    "A": [85,92,78,95,88],
    "B": [20,22,19,23,21]
})
...
# add derived columns
df["Total"] = df["A"] + df["B"] * 0.5
df["Grade"] = df["A"].apply(lambda x: "Excellent" if x>90 else ("Good" if x>80 else "Pass"))
print(df)`,
            insyra: `// Insyra CCL: use AddColUsingCCL with CASE function
...
dt.AddColUsingCCL("Grade", "CASE(A > 90, 'Excellent', A > 80, 'Good', 'Pass')")
dt.Show()`
        },
